...
export function getSquidEntities(schema) {
	...
	const nonPassing = [...entities.entries()].filter(e => e[1]<3).map(e => e[0])
	...
	const confirmedEntities = [...entities.entries()].filter(e => e[1]===3).map(e => e[0])
	const entityQueries = new Set([...confirmedEntities].map(e => [`${squidPluralize(e)}`, `${e}ById`, `${squidPluralize(e)}Connection`]).reduce((a, v) => a.concat(v), []))
	return {
