...
import { squidPluralize, subgraphPluralize, pluralize } from '../entities.js'
...
export function testTemporalEntitiesOnAscendingRecords(
	...
	function getQuery(entities, ename, queryFields, orderByField, kind) {
		...
		if (kind==='squid')
			return {
				query: `{ ${squidPluralize(apiEName)}(limit: ${numRecords}, orderBy: [${orderByField}_ASC, id_ASC]) { ${queryFields.join(' ')} } }`,
				apiEName
			}
		if (kind==='subgraph')
			return {
				query: `{ ${subgraphPluralize(apiEName)}(first: ${numRecords}, orderBy: ${orderByField}, orderDirection: asc, orderBy: id, orderDirection: asc) { ${queryFields.join(' ')} } }`,
				apiEName
			}
	...
	for (let [ename, efields] of temporalReferenceEntities) {
		...
		const referenceResponse = rawReferenceResponse.data[pluralize(referenceApiEName, referenceKind)]
		const sampleResponse = rawSampleResponse.data[pluralize(sampleApiEName, sampleKind)]
...
export function testNonTemporalEntitiesOnCrossInclusion(
	...
	function getSomeRecordsQuery(entityName, queryFields, kind) {
		if (kind==='squid')
			return `{ ${squidPluralize(entityName)}(limit: ${numRecords}) { ${queryFields.join(' ')} } }`
		if (kind==='subgraph')
			return `{ ${subgraphPluralize(entityName)}(first: ${numRecords}) { ${queryFields.join(' ')} } }`
	...
	function checkRecordsInclusion(referenceEntityName, queryFields, issues, direction) {
		...
		const someRecords = direction==='referenceToSample' ?
			queryEndpoint(referenceApiUrl, someRecordsQuery).data[pluralize(referenceEntityName, referenceKind)] :
			queryEndpoint(sampleApiUrl, someRecordsQuery).data[pluralize(sampleEntityName, sampleKind)]
		if (someRecords.length===0) {
			issues.push(`${direction==='referenceToSample' ? 'reference' : 'sample'} API did not return any records (requested ${numRecords})`)
			return
		}
		...
		const inclusionQueryApiFormat = direction==='referenceToSample' ? sampleKind : referenceKind
		const inclusionQuery = `{ ${pluralize(inclusionQueryEntityName, inclusionQueryApiFormat)}(where: {id_in: [${someRecords.map(r => `"${idTransform(r.id)}"`).join(', ')}]}) { ${queryFields.join(' ')} } }`
		const includedRecords =
			direction==='referenceToSample' ?
			queryEndpoint(sampleApiUrl, inclusionQuery).data[pluralize(sampleEntityName, sampleKind)] :
			queryEndpoint(referenceApiUrl, inclusionQuery).data[pluralize(referenceEntityName, referenceKind)]
