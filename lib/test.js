...
export function testTemporalEntitiesOnAscendingRecords(
	...
	for (let [ename, efields] of temporalSubgraphEntities) {
		...
		const issues = []
		if (subgraphResponse.length===squidResponse.length) {
			for (let [i, rec] of subgraphResponse.entries()) {
				for (let f of queryFields) {
					if (f==='id' && ignoreIds) {
						continue
					}
					if (squidResponse[i]==null) {
						issues.push(`for record ${i} the corresponding squid record was not found`)
					}
					else if (rec[f]!=squidResponse[i][f]) {
			...
			issues.push(`response lengths are inconsistent: ${subgraphResponse.length} items from subgraph, ${squidResponse.length} items from squid`)
		}
